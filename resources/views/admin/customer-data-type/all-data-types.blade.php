...
<div class="container-fluid mt-n10">
    <div class="card mb-4 card-header-actions">
        <div class="card-header">All Custom Data Types
            @if($u->hasRole(\App\Role::ADMIN_ROLE_CREATE_DATA_TYPE))
            <a href="{{route('admin.settings.dynamic-data-type.add')}}" class="btn btn-primary btn-sm">
                    ...
                    <tbody>
                        ...
                        @endphp
                            <tr>
                                    ...
                                    <span>
                                        <div class="dropup">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="{{$b1id}}">
                                                @if($u->hasRole(\App\Role::ADMIN_ROLE_EDIT_DATA_TYPE))
                                                <a class="dropdown-item" href="{{ route('admin.settings.dynamic-data-type.edit', ['c_data'=>$item->id]) }}">Edit</a>
                                                @endif
                                                @if($u->hasRole(\App\Role::ADMIN_ROLE_DELETE_DATA_TYPE))
                                                <form action="{{ route('admin.settings.dynamic-data-type.delete.post', ['c_data'=>$item->id]) }}" method="post">
                                                    @csrf
                                                    <button class="dropdown-item" type="button" onclick="ConfirmDelete(this)">Delete</button>
                                                </form>
                                                @endif
                                                </div>
                                        </div>
                                ...
                                <td>{{ $item->title}}</td>
                                <td>{{ $item->slug}}</td>
                                <td>{{ $item->total_fields}}</td>
                                <td>{{ $item->is_optional?"Optional":"Required"}}</td>
                                <td>{{ $item->count>0?$item->count:"Unlimited"}}</td>
                            </tr>
                        @endforeach
                        @else
                        <tr>
...
@push('scripts')
 <script>
     function ConfirmDelete(btn){
        if(confirm('You Want To Delete The Data Type. Are You Sure?')){
