...
<div class="container-fluid mt-n10">
  <div class="card mb-4">
    ...
    <div class="card-body row no-gutters">
      ...
      <div class="col-md-8">
        ...
        <form method="post" action="">
          ...
          <div class="formContainer-x">
            @foreach ($DataTypeJson as $sectIndex => $section)
            @php
              $sect_count = $section['count'];
                            ...
                            @endphp
                             <div class="from-group cd-from-group col-md-6 p-3"  data-data-type="@json_e($fldSchema)" >
                                ...
                                @php
                                  $actualValue = $fldSchema['default'];
                                @endphp
                                <textarea class="form-control cd-control-input" placeholder="{{$fldSchema['child_title']}}" title="{{$fldSchema['child_title']}}" value="{{$actualValue}}" id="{{$main_input_id}}" name="{{$main_input_slug}}"  >{{$actualValue}}</textarea>
                                ...
                                <div class="border p-2 cd-control" title="{{$fldSchema['child_title']}}">
                                  @foreach ($options as $inpOptIndex => $inpOpt)
                                    ...
                                    $eachOptionId = $main_input_id . "-" . $inpOptIndex;
                                        $inpOptLbl = $inpOpt['value'];
                                        $isSelectedText = "";
                                        if(!is_null($actualValue) && in_array($actualValue, $inpOptLbl)){
                                ...
                                <input type="hidden" class="hidden_type" value="textbox" hidden="" name="{{$type_input_slug}}">
                                <input type="hidden" class="hidden_id" value="1" hidden="" name="{{$type_id_input_slug}}">
                             </div>
                            @endforeach
                            <!--
      ...
      <div class="col-md-4">
        <pre>@json_e($DataTypeJson, JSON_PRETTY_PRINT)</pre>
      </div>
